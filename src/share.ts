...
const DEFAULT_SHARE_OUT_BASE_URL = "https://open-design.ai/share-out";
...
// Wrap any outbound X intent URL through /share-out/:eventId so we can count
// "Share on X" button clicks (funnel step 1) without needing the X API.
export function trackedShareOutUrl(
  destinationUrl: string,
  eventId: string,
  lang: "en" | "cn" = "en",
  baseUrl = process.env.SHARE_OUT_BASE_URL || DEFAULT_SHARE_OUT_BASE_URL,
): string {
  const base = `${baseUrl.replace(/\/$/, "")}/${encodeURIComponent(eventId)}`;
...
export function xShareUrl(
  ...
  return trackedShareOutUrl(intentUrl, eventId, lang);
}
